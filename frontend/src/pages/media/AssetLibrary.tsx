...
export function AssetLibrary({ projectId, onClose, onAdd, onRemoved, beforeRemove }: { projectId?: string; onClose: () => void; onAdd?: (asset: Asset) => Promise<void>; onRemoved?: () => Promise<void>; beforeRemove?: () => Promise<void> }) {
  ...
  return <section className="media-drawer" role="dialog" aria-label="素材库" onKeyDown={(event) => { if (event.key === 'Escape') { event.stopPropagation(); onClose() } }}>
    <div className="media-drawer-heading"><div><small>YOUR MATERIALS</small><h2>素材库</h2></div><button ref={close} onClick={onClose} aria-label="关闭素材库">✕</button></div>
    ...
    {loading ? <p role="status">加载素材…</p> : items.length === 0 ? <div className="media-library-empty">{query || kind ? '没有匹配的素材' : '素材库还是空的'}<p>导入图片、视频或音频开始创作。</p></div> : <div className="media-library-grid">{items.map((asset) => <article key={asset.id}>
      ...
      <div className="media-asset-actions">{onAdd && <button disabled={busy} onClick={() => void run(() => onAdd(asset))}>加入画布</button>}{!asset.in_library && <button disabled={busy} onClick={() => void run(() => api(`/assets/${asset.id}/library`, 'POST'))}>加入账户库</button>}
